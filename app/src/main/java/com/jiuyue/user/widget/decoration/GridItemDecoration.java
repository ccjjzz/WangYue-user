...
public abstract class GridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDraw(@NotNull Canvas c, RecyclerView parent, @NotNull RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount1; i++) {
            ...
            boolean[] sideOffsetBooleans = getItemSidesIsHaveOffsets(child,itemPosition);
    ...
    @Override
    public void getItemOffsets(Rect outRect, View view, @NotNull RecyclerView parent, @NotNull RecyclerView.State state) {
        //outRect 看源码可知这里只是把Rect类型的outRect作为一个封装了left,right,top,bottom的数据结构,
        //作为传递left,right,top,bottom的偏移值来用的
        int itemPosition = ((RecyclerView.LayoutParams) view.getLayoutParams()).getViewLayoutPosition();
        boolean[] sideOffsetBooleans = getItemSidesIsHaveOffsets(view, itemPosition);
     ...
     * @return boolean[4]
     */
    public abstract boolean[] getItemSidesIsHaveOffsets(View view, int itemPosition);
}