...
public class ConversationPresenter {
    ...
    /**
     * 在拉取会话列表之前，增加自定义的会话列表
     */
    public void setLoadedConversationInfoList(List<ConversationInfo> conversationInfoList) {
        this.loadedConversationInfoList.addAll(conversationInfoList);
    }
    ...
    public void onNewConversation(List<ConversationInfo> conversationInfoList) {
        ...
        //由于有自定义的订单消息和客服消息会话，这里不能再return
        if (infos.size() == 0) {
        ...
        while (iterator.hasNext()) {
            ...
            if (config.getHideImContactId().equals(update.getId())) {
                iterator.remove();
            }
            //遍历已存在的会话，同新会话集合比较去重
            for (int i = 0; i < loadedConversationInfoList.size(); i++) {
                ...
                if (cacheInfo.getConversationId().equals(update.getConversationId())) {
                    updateOrdersAndCustomerServiceMsg(update);
        ...
        if (adapter != null) {
            Collections.sort(loadedConversationInfoList);
            Logger.i("onNewConversation loadedConversationInfoList:" + loadedConversationInfoList);
            adapter.onDataSourceChanged(loadedConversationInfoList);
    ...
    /**
     * 本地处理订单消息和客服消息默认置顶or头像icon
     *
     * @param update 消息内容
     */
    private void updateOrdersAndCustomerServiceMsg(ConversationInfo update) {
        ConfigEntity config = App.getSharePre().getObject(SpKey.CONFIG_INFO, ConfigEntity.class);
        if (config.getOrderMsgImId().equals(update.getId())) {
            update.setOrderKey(100001);
            update.setTop(true);
            update.setIconPath("");
            update.setIconRes(R.drawable.ic_news_dd);
        } else if (config.getCustomServiceImId().equals(update.getId())) {
            update.setOrderKey(100000);
            update.setTop(true);
            update.setIconPath("");
            update.setIconRes(R.drawable.ic_news_kf);
        }
    }
